...
class EngineCore:
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        inf_end=time.perf_counter()-start
        max_value=0
        if scheduler_output.num_scheduled_tokens:
            max_value=max(scheduler_output.num_scheduled_tokens.values())
        
        import json
        log_data = {
            "total_num_scheduled_tokens": scheduler_output.total_num_scheduled_tokens,
            "num_scheduled_requests":len(scheduler_output.num_scheduled_tokens),  
            "max_num_scheduled_tokens": max_value ,
            "time":inf_end,
            "num_scheduled_tokens": scheduler_output.num_scheduled_tokens,
        }
        with open("tem0_30_spec_1.json", "a", encoding="utf-8") as f:
