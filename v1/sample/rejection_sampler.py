...
MAX_SPEC_LEN = 1024
# Move the kernel outside the class definition

@triton.jit
def parse_output_kernel(
    output_token_ids_ptr,  # [batch_size, max_spec_len + 1]
    valid_counts_ptr,      # [batch_size]
    parsed_tokens_ptr,     # [batch_size * max_valid_tokens]
    vocab_size,
    max_spec_len,
    ...
    # 计算当前batch的起始位置
    row_start = batch_idx * (max_spec_len + 1)
    
    # 统计有效token数量并提取
    valid_count = 0
    for i in range(max_spec_len + 1):
        token_id = tl.load(output_token_ids_ptr + row_start + i)
        if token_id != PLACEHOLDER_TOKEN_ID and token_id < vocab_size:
            # 存储有效token
            output_idx = batch_idx * max_spec_len + valid_count
            tl.store(parsed_tokens_ptr + output_idx, token_id)
            valid_count += 1
    
    # 存储有效token数量
    tl.store(valid_counts_ptr + batch_idx, valid_count)
...
class RejectionSampler(nn.Module):
    ...
    #     vocab_size: int,
    # ) -> list[list[int]]:
    #     """Parse using optimized Triton kernel."""
        
    #     batch_size, max_spec_len_plus_one = output_token_ids.shape
    #     max_spec_len = max_spec_len_plus_one - 1
        
    #     # 分配输出缓冲区
    #     valid_counts = torch.zeros(batch_size, dtype=torch.int32, device=output_token_ids.device)
    #     parsed_tokens = torch.zeros(batch_size * max_spec_len, dtype=torch.int32, device=output_token_ids.device)
        
    #     # 启动kernel
    #     parse_output_kernel[(batch_size,)](
    #         output_token_ids,
    #         valid_counts,
    #         parsed_tokens,
    #         vocab_size,
    ...
    #     parsed_tokens_cpu = parsed_tokens.cpu().numpy()
        
    #     # 构建最终结果
    #     outputs = []
    #     for i in range(batch_size):
    #         count = valid_counts_cpu[i]
    #         if count > 0:
    #             start_idx = i * max_spec_len
    #             tokens = parsed_tokens_cpu[start_idx:start_idx + count].tolist()
