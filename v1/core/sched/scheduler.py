...
class Scheduler(SchedulerInterface):
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        scheduled_spec_decode_tokens_num: dict[str, list[int]] = {}
        # For logging.
        scheduled_timestamp = time.monotonic()
        # have_prefilling : bool = False
        ...
        #     draft_len=len(request.spec_token_ids)
        #     if len(request.spec_token_ids)!=0:
        #         num_spec+=1
        #         max_spec=max(max_spec,draft_len)
        #     req_index += 1

        # # 检查是否需要移除所有草稿
        # # 定义草稿数量阈值，可以根据需要调整
        # SPEC_THRESHOLD = 1  # 可以根据实际需求调整这个阈值
        
        # # 检查prefilling阶段请求
        # has_prefilling_request = False
        # for request in self.running:
        #     num_new_tokens = (request.num_tokens_with_spec - request.num_computed_tokens)
        #     if (0 < self.scheduler_config.long_prefill_token_threshold < num_new_tokens):
        #         num_new_tokens = self.scheduler_config.long_prefill_token_threshold
        #     if len(request.spec_token_ids) != num_new_tokens - 1:
        #         has_prefilling_request = True
        #         break
        
        # # 如果没有prefilling请求且草稿请求数量较少，移除所有草稿
        # if self.running and not has_prefilling_request and num_spec/len(self.running) < SPEC_THRESHOLD:
        #     for request in self.running:
        #         if request.spec_token_ids:
        #             request.spec_token_ids.clear()  # 清空草稿token列表
        #     # 重置统计
        #     num_spec = 0
        #     max_spec = 0
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            req_index += 1
            scheduled_spec_decode_tokens_num[request.request_id] = request.sd_num
        ...
        # if have_prefilling:
        #     break
        # elif num_spec/len(self.running) < 0.5:
        #     for req_id in num_scheduled_spec_tokens:
        ...
        resumed_reqs_data = [
            self._make_cached_request_data(
                ...
                len(optimized_scheduled_spec_decode_tokens.get(req.request_id, ())),
                req_to_new_block_ids[req.request_id],
        ...
        scheduler_output = SchedulerOutput(
            ...
            sd_num=scheduled_spec_decode_tokens_num,
    ...
    def update_from_output(
        ...
        sd_num=model_runner_output.sd_num
