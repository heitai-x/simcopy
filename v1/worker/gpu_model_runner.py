...
from vllm.v1.spec_decode.myngram import MYNgramProposer
...
class GPUModelRunner(LoRAModelRunnerMixin):
    ...
    def __init__(
        ...
        if self.speculative_config and get_pp_group().is_last_rank:
            ...
            if self.speculative_config.method=="myngram":
                self.drafter=MYNgramProposer(self.vllm_config)
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> None:
        ...
        for new_req_data in scheduler_output.scheduled_new_reqs:
            ...
            if sampling_params.sampling_type == SamplingType.RANDOM_SEED:
                ...
                generator = None
            self.sd_num[req_id] = new_req_data.sd_num
    ...
    def execute_model(
        ...
        if max_gen_len == 1:
            ...
            for i, req_id in enumerate(self.input_batch.req_ids):
                if req_id in self.requests and i < len(spec_decode_metadata.num_draft_tokens) and spec_decode_metadata.num_draft_tokens[i] > 0:
                    # print("pre_sd_num:",self.sd_num[req_id])
                    if len(valid_sampled_token_ids[i])-1 ==self.sd_num[req_id][-1]:
                        self.sd_num[req_id][-1] =min(self.sd_num[req_id][-1]+2,10)
                    else:
                        self.sd_num[req_id][-1] =max(1,len(valid_sampled_token_ids[i])-1)
        ...
        elif self.speculative_config.method=="myngram":
            assert isinstance(self.drafter,MYNgramProposer)
            spec_token_ids = self.generate_draft_token_ids(
                valid_sampled_token_ids,sampling_metadata)
    ...
    def generate_draft_token_ids(
        ...
        for i, sampled_ids in enumerate(sampled_token_ids):
            ...
            drafter_output = self.drafter.propose(
                context_token_ids=self.input_batch.token_ids_cpu[i, :end_idx],request_id=req_id,k=sd_num[-1])
            # context_token_ids=self.input_batch.token_ids_cpu[i, :end_idx],k=2)
