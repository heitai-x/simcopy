...
def cascade_attention(
    ...
    # prefix_end_event = torch.cuda.Event(enable_timing=True)
    
    # prefix_start_event.record()
    ...
    # prefix_end_event.record()
    # torch.cuda.synchronize()
    # prefix_time = prefix_start_event.elapsed_time(prefix_end_event)
    # print(f"FlashAttention Prefix processing time: {prefix_time:.4f} ms")

    descale_shape = (cu_query_lens.shape[0] - 1, key_cache.shape[-2])
    # suffix_start_event = torch.cuda.Event(enable_timing=True)
    # suffix_end_event = torch.cuda.Event(enable_timing=True)
    
    # suffix_start_event.record()
    ...
    # suffix_end_event.record()
    # torch.cuda.synchronize()
    # suffix_time = suffix_start_event.elapsed_time(suffix_end_event)
    # print(f"FlashAttention Suffix processing time: {suffix_time:.4f} ms")
