...
class FlashInferImpl(AttentionImpl):
    ...
    def forward(
        ...
        if prefill_wrapper := attn_metadata.prefill_wrapper:
            # # 创建 CUDA 事件来测量 prefill wrapper 运行时间
            ...
            # print(f"Prefill wrapper CUDA runtime: {prefill_time:.4f} ms")

        if decode_wrapper := attn_metadata.decode_wrapper:
            # # 创建 CUDA 事件来测量 decode wrapper 运行时间
            ...
            # print(f"Decode wrapper CUDA runtime: {decode_time:.4f} ms")
