...
SpeculativeMethod = Literal["ngram", "myngram","eagle", "eagle3", "medusa",
...
class SpeculativeConfig:
    ...
    # global_manager: Optional["GlobalResourceManager"] = None
    # """Global resource manager for accessing historical requests and cache data during speculative decoding."""
    ...
    def __post_init__(self):
        ...
        if self.method is None and (self.model is not None
            ...
            self.method = "ngram"
        if self.method is None and (self.model is not None
                                    and self.model == "myngram"):
            self.method = "myngram"
        
        if self.method in ("ngram", "[ngram]","myngram"):
            if self.method in ("ngram","[ngram]"):
            # Unified to "ngram" internally
                self.method = "ngram"
            # Set default values if not provided
            if self.method== "myngram":
                pass
    ...
    def __repr__(self) -> str:
        method = self.method
        model = None if (method == "ngram" or method == "myngram") else self.draft_model_config.model
